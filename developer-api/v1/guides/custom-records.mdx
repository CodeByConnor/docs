...
<img src="/images/Guides/Custom-records-img.png" alt="Custom records" width="450" />
...
- States (Custom Table)
- Has one `region` (reference to the Regions table)
...
- Can have many Regional Directors (reference to the Users table)
...
- Reference columns
  - `native_reference` to a Native Table, linking to core Ramp objects (users, departments, locations, etc.)
  - `custom_reference` to a Custom Table, linking to other custom tables you've created

When writing a reference value:

- Native table references require both Ramp `column_name` and Ramp object `value` (for example, `{"column_name": "id", "value": "<ramp_user_uuid>"}`)
- Custom table references require only the external key
...
- `PUT` operations on columns that reference multiple items (lists) replace all existing values with the provided values. To modify list contents without replacing everything, use the `-/append` and `-/remove` endpoints.
- A `PUT` operation with a `null` contents value removes all values from that field.
...
Using the example Custom Table schema for Regions from above, let's set up the Northeast region to have one regional director (`alice@company.com`) and Southwest region to have two regional directors (`01956422-6793-7700-adb4-3f1bd635ee49` and `bob@company.com`).
      ...
      "cells": [
        {
          ...
          "contents": {
            "column_name": "id",
            "value": "01956422-6793-7700-adb4-3f1bd635ee49"
          }
        },
        {
          "name": "regional_directors",
...
Example query for information about a specific Ramp user:
...
- Reference Column: The column you create (for example, `home_state` on Users)
- Corresponding Column: Automatically created on the referenced table (for example, `residents` on States)

Key points about corresponding columns:

- `corresponding_column_name` is required when creating a reference
- `corresponding_column_label` is optional and auto-generated if not provided
...
When a reference column's contents have `has_more` as `true`, it indicates that not all referenced rows are included in the response. To retrieve the complete set of references, query the referenced table directly using the corresponding column.

For example, to get all states in the West region, query the states table using the corresponding `region` column.

## Supported Native Tables

Note that when providing a `contents` value that references a Ramp object, it should be formatted similarly to: `{"column_name": "id", "value": "<UUID>"}`. Different Native Ramp tables allow other column names to be used as well.
...
Users endpoint: `GET /developer/v1/users`
...
Locations endpoint: `GET /developer/v1/locations`
...
Departments endpoint: `GET /developer/v1/departments`
...
Make sure that if you're using this format, you provide the exact `field_id::option_id` string as returned by Ramp.

Accounting Field Options endpoint: `GET /developer/v1/accounting/field-options`
...
| `name` | Entity's name | `{"column_name": "name", "value": "Acme Corp LLC"}` |

Entities endpoint: `GET /developer/v1/businesses`
...
Bills endpoint: `GET /developer/v1/bills`
...
Transactions endpoint: `GET /developer/v1/transactions`
...
Reimbursements endpoint: `GET /developer/v1/reimbursements`
